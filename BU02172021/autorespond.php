...
function appointmentFields($cid, $dcid, $date, $time, $location) {
	global $dc; //, '_AppointmentDate', '_AppointmentTime', '_AppointmentOfficeLocation', '_DrChronoId', '_AppointmentId1';
	return $dc->keap->dsUpdate('Contact', $cid, array('_AppointmentDate'=>$date, '_DrChronoId'=>$dcid,
		'_AppointmentTime'=>$time, '_AppointmentOfficeLocation'=>$location, '_AppointmentId'=>$dc->obj->object->id));
}
...
if (($event==='APPOINTMENT_CREATE') ||
	...
	if (($event!=='APPOINTMENT_DELETE') && (!$obj->appt_is_break)) {
		...
		$dc->keap->dsUpdate('Contact', $cid, array('_AppointmentDate'=>'',
			'_AppointmentTime'=>'', '_AppointmentOfficeLocation'=>'', '_AppointmentId1'=>''));
	}
	if ($obj->status!=='') {
