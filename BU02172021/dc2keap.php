...
class dc2keapObj {
	...
	function keapContactAdd($id) {
		...
		$cf=$this->keap->dsUpdate('Contact', $cid, array('_DrChronoId'=>$id));
		if (!is_integer($cf)) {
			$this->setError(448);
	...
	function getContactByDCId($id) {
		global $DRChronoId;
		$result=$this->keap->dsFind('Contact', 1, 0, '_DrChronoId', $id, array('Id',
									'FirstName', 'MiddleName', 'LastName', 'Email',
									'Nickname', 'Phone1Type', 'Phone1', 'Phone2Type', 'Phone2',
									'Phone3Type', 'Phone3', 'StreetAddress1', 'City', 'State',
									'PostalCode', 'Birthday', '_DrChronoId'));
		/**$result = $this->keap->dsQuery('Contact', 1, 0, array('_DrChronoId'=>$id),
									array('Id', 'FirstName', 'MiddleName', 'LastName', 'Email',
										'Nickname', 'Phone1Type', 'Phone1', 'Phone2Type', 'Phone2',
										'Phone3Type', 'Phone3', 'StreetAddress1', 'City', 'State',
										'PostalCode', 'Birthday', '_DrChronoId'));**/
		/**if (!is_integer($result)) $this->log->lfWriteLn('Error in getContactByDCId($id) = '.$result);
		$this->log->lfWriteLn('$result = '.$result[0]['Id']);
		if ((is_array($result)) && (empty($result))) return false; else return $result[0]['Id'];**/
		
		if (is_array($result)) {
			// if empty then not found and return false, otherwise, return the contact id
			if (count($result)!=1) {
				$this->log->lfWriteLn('getContactByDCId($id) = (not found)');
				return false;
			} else {
				$this->log->lfWriteLn('getContactByDCId($id) = found .. id = '.$result[0]['Id']);
				return ($result[0]['Id']);
			}
		} else {
			// this is an error case
			$this->log->lfWriteLn('getContactByDCId($id) error = '.$result);
			return false;
		}
	}
